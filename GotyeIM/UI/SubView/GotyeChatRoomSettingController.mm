...
@interface GotyeChatRoomSettingController () <GotyeUIDelegate>

@end

@implementation GotyeChatRoomSettingController

- (id)initWithTarget:(gotyeapi::GotyeRoom)target
{
    self = [self init];
    if(self)
    {
...
- (void)viewDidLoad
{
    ...
    apiist->reqRoomMemberList(roomTarget, 0);
...
- (void)swichNickName
{
    ...
    NSInteger IconRow = (roomUserlist.size() + 1 + 4) / 5;
    for(int i = 0; i<roomUserlist.size(); i++)
    {
        GotyeUser user = roomUserlist[i];
...
- (void)onGetRoomMemberList:(GotyeStatusCode)code room:(gotyeapi::GotyeRoom)room pageIndex:(unsigned int)pageIndex curPageMemberList:(std::vector<GotyeUser>)curPageMemberList allMemberList:(std::vector<GotyeUser>)allMemberList
{
    ...
    BOOL showNickName = [roomSetting[Setting_key_NickName] boolValue];
    NSInteger IconRow = (roomUserlist.size() + 4) / 5;
    headListView.frame = CGRectMake(0, 0, 320, IconGap + (IconSize + IconGap + (showNickName ? IconNameHeight : 0)) * IconRow);
    ...
    for(int i = 0; i<roomUserlist.size(); i++)
    {
        ...
        NSString *displayName = [roomSetting[Setting_key_NickName] boolValue] ? NSStringUTF8(user.name): @"";
