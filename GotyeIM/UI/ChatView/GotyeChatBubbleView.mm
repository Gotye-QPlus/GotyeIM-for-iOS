...
+(NSInteger)getbubbleHeight:(GotyeMessage)message showDate:(BOOL)showDate
{
    ...
    switch (message.type) {
        ...
        {
            size = [NSStringUTF8(message.text) boundingRectWithSize:CGSizeMake(kContentWidthMax, 1000.0f)
        ...
        {
            UIImage* image = [UIImage imageWithContentsOfFile:NSStringUTF8(message.media.path)];
            size = image.size;
            size.width /= 2;
            size.height /= 2;
...
{
    NSString *text = NSStringUTF8(message.text);
    ...
    BOOL msgFromSelf = (apiist->getLoginUser().name.compare(message.sender.name) == 0);
...
//  头像图片
    GotyeUser user = apiist->getUserDetail(message.sender);
    NSString *headpath = NSStringUTF8(user.icon.path);
